...
Pipeline integrado GRR:
  ...
  2. rrg_ls_mapper genera la Estructura Lógica (LS) con pipeline de 3 niveles:
       ...
       Nivel 3 — fallback LLM local (rrg_llm_fallback, via Ollama)
...
Modos de uso:
  python3 rrg_interactivo.py                        # interactivo: bucle oración por oración
  python3 rrg_interactivo.py --batch                # batch: ingresar varias oraciones en terminal
  python3 rrg_interactivo.py --batch oraciones.txt  # batch: cargar desde archivo .txt
  python3 rrg_interactivo.py archivo.conllu         # leer .conllu ya anotado con rrg_*

Modo interactivo:
  Escribe una oración, ve el análisis, repite. Escribe 'salir' para terminar.
  Al salir se ofrece guardar toda la sesión en un solo .txt.

Modo batch desde terminal:
  Escribe una oración por línea. Escribe FIN (o dos Enters) para terminar.
  Al final se ofrece guardar todo en un solo .txt.

Formato archivo .txt para batch:
  Una oración por línea. Líneas con # se ignoran. Líneas vacías se omiten.
"""
...
def _bloque_ls_txt(ls: dict) -> str:
    ...
    if src and src != 'lexicon':
        etiquetas = {'morph': 'morfosintaxis', 'llm': 'LLM local (gemma)'}
...
def _guardar_resultados(nombre_archivo: str, titulo: str, origen: str,
    ...
    """Núcleo de guardado compartido por guardar_analisis y guardar_batch."""
    ...
    for n, res in enumerate(resultados, start=1):
        ...
        bloques.append(encabezado_oracion + cuerpo)

    with open(nombre_archivo, 'w', encoding='utf-8') as f:
        f.write(_encabezado_txt(titulo, origen, fecha, extra))
        f.write("\n".join(bloques))

    print(f"\n[OK] Guardado en: {nombre_archivo}")


def guardar_analisis(origen: str, ls_data_lista: list, stdout_ud2rrg: str):
    """Guarda el análisis de una oración (modo interactivo)."""
    res = [{'oracion': origen, 'ls_lista': ls_data_lista, 'stdout': stdout_ud2rrg}]
    _guardar_resultados(_nombre_archivo(origen),
                        "ANÁLISIS GRR — ud2rrg + rrg_ls_mapper",
                        origen, res)


def guardar_batch(origen: str, resultados: list[dict]):
    """Guarda el análisis consolidado de un batch o sesión completa."""
    _guardar_resultados(_nombre_archivo_batch(origen),
...
def procesar_oracion(nlp, oracion: str) -> dict:
    """Pasa una oración por el pipeline completo. Retorna dict con resultados."""
    doc = nlp(oracion)
...
def mostrar_resultado(res: dict, n: int = 1, total: int = 1):
    """Imprime el análisis completo de una oración en pantalla."""
...
def procesar_batch(nlp, oraciones: list[str]) -> list[dict]:
    total      = len(oraciones)
    resultados = []
    print(f"\n{SEP}\n  PROCESANDO BATCH ({total} oración(es))\n{SEP}")
...
def main():
    ...
    if args and args[0].endswith('.conllu') and os.path.exists(args[0]):
        ...
        print(f"{SEP}\n  ANÁLISIS GRR — Modo archivo CoNLL-U\n  Fuente: {nombre}\n{SEP}")
        ...
        print()
        if input("¿Guardar en .txt? (s/n): ").strip().lower() in ("s","si","sí","y","yes"):
    ...
    if args and args[0] == '--batch':
        ...
        print(f"{SEP}\n  ANÁLISIS GRR — MODO BATCH\n{SEP}")

        if archivo_txt:
            if not os.path.exists(archivo_txt):
                print(f"[ERROR] No se encontró: {archivo_txt}")
                sys.exit(1)
            print(f"\n  Cargando desde: {archivo_txt}")
        ...
        print(f"\n[OK] Listo. Procesando {len(oraciones)} oración(es)...\n")

        resultados = procesar_batch(nlp, oraciones)

        print(f"\n{SEP}\n  BATCH COMPLETADO — {len(resultados)} oración(es)\n{SEP}\n")
        if input("¿Guardar el análisis completo en .txt? (s/n): ").strip().lower() in ("s","si","sí","y","yes"):
    ...
    while True:
        ...
        if oracion.lower() in ['salir', 'exit', 'quit']:
            if sesion_resultados:
                print(f"\n  ({len(sesion_resultados)} oración(es) analizadas en esta sesión)")
                if input("¿Guardar toda la sesión en .txt? (s/n): ").strip().lower() in ("s","si","sí","y","yes"):
                    guardar_batch('sesion_interactiva', sesion_resultados)
        ...
        # LS detallado (con fuente y rasgos morph)
        print(f"\n{SEP}\n  ESTRUCTURAS LÓGICAS\n{SEP}")
        ...
        print()
        if input("¿Guardar este análisis en .txt? (s/n): ").strip().lower() in ("s","si","sí","y","yes"):
