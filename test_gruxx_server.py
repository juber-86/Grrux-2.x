"""Tests de `gruxx_server.py` — Fase GUI, Etapa G1.4.
...
servidor real ni a gruxx interactivo).

Ejecutar:
    python -m pytest test_gruxx_server.py
    RUN_SLOW=1 pytest test_gruxx_server.py
"""
...
def _cliente(data_dir=None, config_path=None):
    motor = _MotorStub()
    app = gruxx_server.crear_app(motor=motor, data_dir=data_dir, config_path=config_path)
...
def test_lock_compartido_analizar_y_correccion_no_se_solapan():
    ...
    tmp = _tmp_datos()
    motor = _MotorLento()
    app = gruxx_server.crear_app(motor=motor, data_dir=tmp)
...
def test_slow_analizar_real_ditransitiva_agx():
    ...
    cliente = TestClient(gruxx_server.app)
    with cliente as c:
...
def test_slow_corregir_el_real_con_reanalisis():
    ...
    re-análisis -- motor REAL (`gruxx_motor`, no un stub) pero `data_dir`
    ...
    import gruxx_motor
    tmp = _tmp_datos()
    app = gruxx_server.crear_app(motor=gruxx_motor, data_dir=tmp)
    cliente = TestClient(app)
    try:
        with cliente as c:
...
def test_slow_lote_real_pequeno_y_exportacion_txt():
    ...
    `display_grr.render_bloque`, ver `gruxx_motor.render_txt_grr`) + un
