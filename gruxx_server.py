...
gruxx_server.py
================
Fase GUI, Etapas G1.1 + G2 §2 — servidor local que expone `gruxx_motor` y
...
- El motor se INYECTA (`crear_app(motor=...)`) para poder testear con un
  stub sin cargar Stanza (ver `test_gruxx_server.py`, patrón G1.4);
...
Lanzar (dev, sin el script `gruxx-gui`):
    ./venv/bin/uvicorn gruxx_server:app --host 127.0.0.1 --port 8763
"""
...
import gruxx_motor as _motor_real
...
def crear_app(motor=_motor_real, data_dir=None, config_path=None) -> FastAPI:
    """Fábrica de la app — `motor` inyectable (stub en tests, `gruxx_motor`
    ...
    app = FastAPI(title="gruxx GUI", lifespan=lifespan)
    ...
    def analizar(body: OracionIn):
        ...
        # El motor NUNCA lanza (ver gruxx_motor.analizar): cualquier fallo
    ...
    def corregir_operador_endpoint(body: OperadorIn):
        ...
        re-analizar y/o escribir en config: gruxx decide SOLO si la corrección
    ...
    def exportar_txt_endpoint(analisis_id: str):
        ...
        `gruxx_motor.render_txt_grr`). Puro: no usa el lock (no toca Stanza
        ni persiste nada)."""
