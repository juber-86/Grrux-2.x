...
Uso:
  python3 analizar_texto_ls.py "Juan come pizza."
  python3 analizar_texto_ls.py                      # usa oración de prueba
...
def guardar_analisis(origen: str, ls_data_lista: list, stdout_ud2rrg: str):
    ...
    Parámetros
    ----------
    origen        : texto de la oración (modo texto) o nombre del archivo
                    (modo archivo) — se usa solo para el encabezado.
    ls_data_lista : lista de dicts con los datos LS de cada oración.
    stdout_ud2rrg : salida completa de ud2rrg (puede contener varias oraciones).
    """
...
# ---------------------------------------------------------------------------
def main():
    arg = sys.argv[1] if len(sys.argv) > 1 else "Juan come pizza."

    # ── Bifurcación: ¿archivo .conllu existente o texto libre? ──────────────
    if arg.endswith('.conllu') and os.path.exists(arg):
        modo_archivo = True
    ...
    # ════════════════════════════════════════════════════════════════════════
    # MODO TEXTO — pipeline completo con Stanza
    # ════════════════════════════════════════════════════════════════════════
    ...
    # 2b. Limpiar CoNLL-U de Stanza (lemas, XPOS, MISC, sent_id)
