...
módulo es SOLO orquestación, reutiliza `gruxx_ai1.py` tal cual (mismas
...
diferencias respecto al modo interactivo:
  ...
  2. No se invoca el subprocess `convertir.py` (el ASCII no se necesita en
     ...
     (`gruxx_ai1._arboles_inprocess`) que ya alimenta a `completeness`,
...
def cargar() -> None:
    ...
    `rrg_ls_mapper` (vía `gruxx_ai1`, más abajo) ya deja cargado el
    clasificador aspect_classifier — igual que hace la CLI."""
    global _nlp
    if _nlp is not None:
        return
    import gruxx_ai1  # noqa: F401 — módulo importable sin coste (no construye Stanza)
    _nlp = gruxx_ai1.cargar_pipeline_stanza(LANG)
...
def _pipeline_crudo(oracion: str):
    """Corre el pipeline completo (idéntico al de `gruxx_ai1.procesar_oracion`,
    ...
    fd, tmp_path = tempfile.mkstemp(suffix=".conllu", prefix="gruxx_gui_")
    os.close(fd)
    try:
        doc = _nlp(oracion)
        open(tmp_path, "w", encoding="utf-8").close()
        CoNLL.write_doc2conll(doc, tmp_path)
        gruxx_ai1.limpiar_conllu_stanza(tmp_path)
        ls_lista = [map_sentence_to_ls(sent) for sent in doc.sentences]
        gruxx_ai1.inyectar_metadata_rrg(tmp_path, doc, ls_lista)
        ...
        try:
            arboles = gruxx_ai1._arboles_inprocess(tmp_path, LANG)
        except Exception:
            arboles = []
...
def render_txt_grr(crudo: dict) -> str:
    """Reusa `display_grr.render_bloque` (mismo orden que `gruxx_ai1`: árbol
