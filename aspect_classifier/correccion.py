...
gruxx) señala qué está mal en un análisis; gruxx VALIDA la corrección y la
...
def _pedir(prompt, entrada, salida):
    ...
    prompt de guardar/corregir de gruxx_ai1, arreglado en el mismo commit)."""
    while True:
        salida(prompt)
        try:
            val = entrada("> ")
...
def _enrutar_core(oracion, sub_idx, texto, lema_item, destino_clave, destino_etiqueta,
                  verbo_lema, reanalizar_fn, salida, data_dir, config_path) -> dict:
...
def corregir_enrutado(res, sub_idx, elemento_id, destino, reanalizar_fn,
    ...
    return _enrutar_core(oracion, sub_idx, texto, lema_item, destino, destino_etiqueta,
                         verbo_lema, reanalizar_fn, lambda *_: None, data_dir, config_path)
...
def corregir_operador(res, sub_idx, operador, accion, valor_correcto,
    ...
    `accion` ∈ {'cambiar', 'quitar', 'anadir'}. gruxx decide SOLO la ruta —el
...
def _confirma_enrutado(ls_nuevo, texto, destino_clave):
    ...
    if destino_clave == "argumento_core":
        return any(c.get("text") == texto for c in (ls_nuevo.get("core") or []))
