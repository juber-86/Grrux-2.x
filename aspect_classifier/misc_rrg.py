...
Vocabulario mínimo (namespace `RRG*`, para no chocar con MISC de terceros):
    ...
    RRGAnalyzed=si
        En el token raíz, SIEMPRE que GRRux analizó la oración (L5 §1).
...
def anotaciones_misc(ls: dict) -> dict[int, dict[str, str]]:
    ...
    # analizó GRRux (canal MISC poblado). ud2rrg._es_conllu_analizado la lee
...
def leer_ls_desde_misc(bloque: str) -> dict:
    ...
    de `grrux_ai1.py`, donde no se re-corre el mapper — el `.conllu` ya
...
def escribir_misc_en_conllu(conllu_path: str,
    ...
    `# text = `, igual que `grrux_ai1.inyectar_metadata_rrg`).
    """
    with open(conllu_path, encoding="utf-8") as f:
        lineas = f.readlines()

    # L5 §1: una oración está "analizada" por GRRux si su dict de anotaciones
