"""Fase L5 §3 — traducción user-friendly de la salida de GRRux.

El usuario objetivo es un lingüista competente en la GRR (Role and Reference
Grammar) que NO tiene por qué aprender los tecnicismos internos de GRRux. La
...
detalle crudo completo sigue disponible con `--verbose` (ver grrux_ai1.py).

Los términos y sus definiciones viven en el glosario (§4,
`data/glosario_grrux.csv`) — este módulo usa las MISMAS etiquetas legibles
...
def avisos(ls: dict) -> list[str]:
    """AVISOS (2026-08-19) — `diagnosticos_analisis` del mapper, legibles.

    El mapper ya calculaba estos diagnósticos (p. ej. "argumento 'maria'
    (obl) sin posición licenciada en la EL seleccionada", que es como se
    manifiesta un participante que el enrutado dejó fuera de la EL) y los
    exponía en `ls['diagnosticos_analisis']`, pero NINGÚN consumidor los
    mostraba: ni la terminal, ni el .txt, ni la GUI. Eran datos muertos.
    Con "juan le da un beso a maria" el sistema sabía que maría se había
    quedado sin posición y no lo dijo en ninguna parte.

    Función pura; devuelve [] cuando no hay nada que avisar (caso normal),
    así que la salida de un análisis sano no cambia.
    """
    return [f"aviso: {d}" for d in (ls.get("diagnosticos_analisis") or [])]
