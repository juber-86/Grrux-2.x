...
el conversor del parser):
  ...
  1. Tasa de fallos de ud2rrg (vía el mismo subprocess que usa
     `gruxx_ai1.correr_ud2rrg`: `convertir.py <conllu> es`).
...
`gruxx_ai1.limpiar_conllu_stanza` pone la columna XPOS en `_` para cada
...
tagset. El pipeline real de `gruxx_ai1.py` evita el bug por el efecto
...
gruxx_ai1 — es la cifra comparable con lo que Julian usa día a día, y la
...
`main()`, que la incluye) con una instancia interactiva de `gruxx_ai1.py`
...
# ---------------------------------------------------------------------------
# KPI 1 — tasa de fallos de ud2rrg (subprocess, mismo path que gruxx_ai1)
# ---------------------------------------------------------------------------
def _xpos_stripped(bloque: str) -> str:
    """Replica el efecto de gruxx_ai1.limpiar_conllu_stanza: xpos → '_'."""
    lineas = []
...
def correr_convertir(bloques: list[str], tmp_path: str) -> dict:
    ...
    igual que gruxx_ai1.correr_ud2rrg. Devuelve conteos parseados de stdout."""
...
def main():
    ...
    print(f"  xpos stripped (pipeline real gruxx_ai1): {xs['pct_ok']}% OK / {xs['pct_fail']}% fail")
