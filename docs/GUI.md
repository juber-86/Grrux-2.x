...
(`python3 grrux_ai1.py`), no la reemplaza: los dos modos siguen soportados.
...
La primera vez levanta el servidor (`grrux_server.py`, carga Stanza +
BERTIN — tarda; puede verse el progreso en `grrux-gui.log`) y abre el
...
**No** correr `./grrux-gui` (ni dejarlo abierto) al mismo tiempo que
`python3 grrux_ai1.py` en modo interactivo o que las suites `--slow` —
ambos cargan su propia copia de los mismos modelos.

Para cerrar el servidor: `pkill -f "uvicorn grrux_server:app"` (o matar el
proceso que quedó en `grrux-gui.log`).

## Icono de escritorio (opcional, instalación manual)

El repo trae `grrux.desktop` de ejemplo, ya con `Icon=` apuntando a
...
```
cp grrux.desktop ~/.local/share/applications/grrux.desktop
chmod +x ~/.local/share/applications/grrux.desktop
...
- `grrux_motor.py` — el pipeline de análisis (Stanza → mapper → árbol RRG →
...
- `grrux_server.py` — FastAPI, `127.0.0.1:8763`, un solo worker. Endpoints:
