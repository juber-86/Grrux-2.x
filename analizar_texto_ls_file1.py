...
def _dividir_arboles_ud2rrg(stdout: str) -> list[str]:
    ...
    # El primer elemento es el encabezado "Convirtiendo: ..." — lo descartamos
...
def leer_ls_desde_conllu(conllu_path: str) -> list[dict]:
    ...
    for bloque in content.strip().split('\n\n'):
        ...
        # Solo incluir si tiene al menos LS formal
...
def main():
    ...
    if modo_archivo:
        print(f"  Modo  : archivo CoNLL-U")
        print(f"  Fuente: {nombre}")
    else:
        print(f"  Modo  : texto libre")
    ...
    if modo_archivo:
        ...
        # Dividir stdout de ud2rrg en bloques por oración
        # Separador: línea que empieza con "--- Oración N ---"
        arboles = _dividir_arboles_ud2rrg(stdout)

        # Entrelazar: LS + árbol oración por oración
