...
# ---------------------------------------------------------------------------
# AVISOS (2026-08-19): los `except Exception` de esta cabecera degradaban el
# análisis EN SILENCIO ABSOLUTO. Si el léxico ditransitivo no cargaba (un
# .xlsx bloqueado por Calc basta), `_DITRANS_LEXICON` quedaba en None, la
# plantilla de transferencia dejaba de dispararse y la clase la decidía el
# clasificador — sin una sola línea en pantalla, en el log ni en aspect_note
# que lo dijera. Se conserva la degradación elegante (el mapper sigue
# corriendo), pero ahora DEJA CONSTANCIA: cada fallo entra en
# `diagnosticos_analisis`, que la terminal y la GUI ya muestran.
_LEXICOS_NO_CARGADOS: list[tuple[str, str]] = []


def _avisos_lexicos() -> list[str]:
    """Un aviso por léxico/módulo que no cargó al importar (normalmente [])."""
    return [f"{nombre} no cargó ({error}); esa capa quedó DESACTIVADA en "
            "este análisis — el resultado puede no reflejar el diseño"
            for nombre, error in _LEXICOS_NO_CARGADOS]
...
try:
    ...
    _LEXICOS_NO_CARGADOS.append(("el clasificador aspectual", f"{type(_e).__name__}: {_e}"))
...
try:
    ...
    _CAUS_LEXICON = None
    _LEXICOS_NO_CARGADOS.append(("el léxico causativo", f"{type(_e).__name__}: {_e}"))
...
try:
    from aspect_classifier.ditransitivas import (clase_derivada_de_el,
                                                  construir_ditransitiva,
                                                  cargar_lexicon as
    ...
    _LEXICOS_NO_CARGADOS.append(("el léxico ditransitivo", f"{type(_e).__name__}: {_e}"))
...
def map_sentence_to_ls(sentence) -> dict:
    ...
    if cop_info is not None:
        ...
        salida = {
            ...
            'wrappers': [],
            'diagnosticos_analisis': _avisos_lexicos(),
        }
    ...
    if ditrans is not None:
        ...
        # DITRANS-CLASE (2026-08-19) — contrato de presentación LA2. La rama
        # ditransitiva salta por completo la de causatividad (ver arriba), y
        # `causativo_clase_derivada` solo la poblaba esa rama: resultado, una
        # transferencia se presentaba como "Realización" aunque su propia EL
        # fuera [do'(x, Ø)] CAUSE [BECOME have'(y, z)] — la etiqueta
        # "Realización causativa" existía en display_grr y era INALCANZABLE
        # por esta vía. Se deriva de la EL con la MISMA regla que usa la rama
        # causativa (BECOME→realización, INGR→logro), no con una lista de
        # plantillas: la EL es la fuente de verdad. Las plantillas sin CAUSE
        # (comunicación, benefactiva de subtipo actividad) no derivan nada.
        # No toca ls_type, ni el vector, ni la EL: solo la clase VISIBLE.
        if ditrans_cfg.get('clase_causativa_visible', True):
            caus["clase_derivada"] = clase_derivada_de_el(ls['formal'])
            if caus["clase_derivada"]:
        ...
        # (misma regla que `ditransitivas.clase_derivada_de_el`, que la rama
        # ditransitiva usa; candidata a unificarse en un futuro hito)
    ...
    salida = {
        ...
        'id_a_var':            id_a_var,
        'diagnosticos_analisis': diagnosticos + _avisos_lexicos(),
        'root_id':             root.id,
