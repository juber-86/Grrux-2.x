#!/usr/bin/env bash
# ============================================================================
# instalar_estudiantes.sh — instalador de gruxx para las laptops de los
...
# que todo importe. Al final ofrece una prueba completa con una oración.
# ============================================================================
set -u
...
echo "gruxx — instalador para estudiantes"
echo "  repo:      $REPO"
...
case "$PYVER" in
  ...
  3.11|3.12|3.13)
    aviso "Python $PYVER — gruxx está probado con 3.10 (Mint 21 / Ubuntu 22.04)."
  ...
  *) morir "Python $PYVER es demasiado viejo: gruxx necesita 3.10 o más (Mint 21+ / Ubuntu 22.04+)" ;;
esac
...
# núcleo de la terminal (gruxx_ai1.py y todo su árbol de imports)
requerir gruxx_ai1.py                 "gruxx_ai1.py (la terminal)"
...
requerir aspect_classifier/data/glosario_gruxx.csv       "glosario (-help)"
...
# GUI (fase 2 — no impide instalar la terminal)
GUI_OK=1
for f in gruxx_motor.py gruxx_server.py gui/index.html gui/app.js gui/estilo.css; do
  [ -e "$REPO/$f" ] || GUI_OK=0
done
if [ "$GUI_OK" = 1 ]; then ok "archivos de la GUI (para la fase 2)"; else aviso "faltan archivos de la GUI — la terminal funciona igual; la GUI se instalará después"; fi
...
ok "todos los archivos obligatorios están"
...
echo "  instalando el resto de los paquetes (requirements.txt)…"
...
# ══════════════════════════════════════════════════════════════════════════
paso "6/6 Prueba de humo (imports, sin cargar modelos)"
# ══════════════════════════════════════════════════════════════════════════
...
import gruxx_motor          # arrastra el árbol de imports de gruxx sin cargar Stanza
from aspect_classifier import correccion, glosario, display_grr
print("imports OK")
EOF
); then
  ok "todos los módulos importan"
else
...
echo
echo " Para usar gruxx (desde $REPO):"
echo "   ./venv/bin/python gruxx_ai1.py"
echo
if [ "$MODELOS_FALTAN" = 1 ]; then
  echo " ${AMBAR}OJO:${FIN} faltan modelos — el PRIMER análisis descargará ~2 GB."
  echo
fi
echo " La primera carga de modelos tarda varios minutos en laptops"
echo " lentas; después queda residente mientras el programa esté abierto."
echo " En laptops de 4 GB de RAM: cerrar el navegador antes de usarlo."
echo "${VERDE}════════════════════════════════════════════════════════${FIN}"
echo
read -r -p "¿Correr ahora una prueba completa con una oración real? Carga los modelos, puede tardar minutos [s/N] " R
...
r = gruxx_motor.analizar("Juan le dio un regalo a María")
...
print()
print("PRUEBA COMPLETA SUPERADA — gruxx funciona en esta máquina.")
EOF
