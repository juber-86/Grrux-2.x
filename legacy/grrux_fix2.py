...
def _bloque_ls_txt(ls: dict) -> str:
    ...
    if src:
        etiquetas = {'lexicon': '📖 Léxico estático', 'morph': '🔬 Morfosintaxis', 'llm': '🤖 LLM local'}
...
def imprimir_ls(ls: dict, idx: int):
    ...
    # CORRECCIÓN DE ISSUE 1: Mostrar siempre la fuente
    src = ls.get('cls_source', '')
    if src:
        etiquetas = {'lexicon': '📖 Léxico estático', 'morph': '🔬 Morfosintaxis', 'llm': '🤖 LLM local'}
...
# ---------------------------------------------------------------------------
# BATCH Y SELECCIÓN OMITIDOS DE CAMBIOS MAYORES (Resto igual al original)
# ---------------------------------------------------------------------------
